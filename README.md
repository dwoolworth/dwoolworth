### Hello, I'm Derrick!

Creator and founder of [MnemoShare](https://mnemoshare.com) — a HIPAA-compliant, HITRUST-certifiable MFT platform. I build software that handles sensitive data the right way: application-layer encryption, audit logging, and compliance baked in from day one.  Built for healthcare, legal, and financial services.

I work primarily in **Go**, **Rust**, **C/C++**, **TypeScript**, and **React**, with a strong focus on Kubernetes, infrastructure automation, and security tooling in heavily regulated environments.

[![Website](https://img.shields.io/badge/mnemoshare.com-Visit-blue?style=flat-square)](https://mnemoshare.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/dwoolworth/)
